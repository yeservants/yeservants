...
**Routing (v7 rework, 2026-08-20 — supersedes FINAL v2).** Nav (3 + button): Home · The Missionaries · Contact · **[Give Now]** (orange, always visible incl. mobile — links straight to the external **Aplos** donation form, since `/give/` is hidden). Visible pages: `/` (the v7 landing — hero letter w/ arch carousel → field stories → insight → Why Trust YES stats → Investment quote → 4 Differences → Model comparison table → Partnership areas → Special Projects → Biblical → Final CTA → **missionaries procession carousel**) · `/missionaries/` + `/missionaries/[slug]/` · `/contact/` (John 13:35 closes) · `/privacy/` · `/404`. **Hidden pages** (client directive 2026-08-19: exist at their URLs but are `noindex`, out of the sitemap, and NOTHING may link to them): `/about/` · `/how-it-works/` · `/mission-teams/` · `/give/` · `/join/`. Authoritative content source for the home: the client's v7 landing (EN + ES) — full verbatim text + the directive audios in `docs/client-directives-2026-08-19.md`. If deploying to the production domain where old URLs are indexed, add base-correct **meta-refresh stub pages** (Astro's `redirects` config strips the base from targets and 404s on a base path — don't use it).
